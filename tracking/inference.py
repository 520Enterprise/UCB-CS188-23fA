...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = [i for i in range((X_RANGE - 1) + (Y_RANGE - 1) + 1 + MAX_NOISE)]
    variableDomainsDict[OBS1] = [i for i in range((X_RANGE - 1) + (Y_RANGE - 1) + 1 + MAX_NOISE)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict,
        ...
        # join factors by eliminationOrder and eliminate
        for eliminationVariable in eliminationOrder:
            # join
            # currentFactorsList means all factors that are not eliminated yet (except joinedFactor)
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)
            # eliminate if there are more than one unconditioned variables
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        sumOfValues = self.total()
        if sumOfValues != 0:
            for key in self.keys():
                self[key] = self[key] / sumOfValues
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            return noisyDistance is None
        else:
            if noisyDistance is None:
                return 0
            tureDistance = manhattanDistance(pacmanPosition, ghostPosition)
            return busters.getObservationProbability(noisyDistance, tureDistance)
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            self.beliefs[ghostPosition] *= self.getObservationProb(observation, pacmanPosition, ghostPosition,
                                                                   jailPosition)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldGhostPosition in self.allPositions:
            ...
            oldBelief = self.beliefs[oldGhostPosition]
            for newGhostPosition, probability in newPosDist.items():
                newBeliefs[newGhostPosition] += oldBelief * probability
